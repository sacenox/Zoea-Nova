...
func TestDashboardLayoutCalculations(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			output := RenderDashboard(myses, swarmMsgs, 0, tt.termWidth, contentHeight, loadingSet, spinnerView)
...
func TestFocusViewLayoutCalculations(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Render focus view
			output := RenderFocusViewWithViewport(mysis, vp, tt.termWidth, false, "⬡", true, false, tt.numLogs, 1, 1)
