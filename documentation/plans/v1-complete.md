...
Upstream API doc (gameserver v0.43.0) documents captain’s log limits and behavior.
We should reflect these limits in prompts to prevent avoidable errors.

- [x] **Verify current prompts include limits and examples**
  - SystemPrompt: non-empty entry examples + limits
  - ContinuePrompt: non-empty reminder + limits
  - Limits from https://www.spacemolt.com/api.md:
    - Max 20 entries per player (oldest entry evicted when full)
    - Max 100KB per entry
    - Commands are query-only (not rate-limited)

- [x] **Verify empty_entry behavior still occurs**
  - Test with fresh mysis and empty entry
  - If behavior changed, update error handling in mysis tool result formatting

**Completed:** 2026-02-05 21:10 UTC
...
- [x] **Add provider tests**
  - Test Ollama and OpenCode error handling paths
  - Test shared rate limiter behavior
  - Mock HTTP responses for tool calls
  - Completed: 2026-02-05 21:30 UTC

- [x] **Add config validation tests**
  - Test environment override precedence
  - Test invalid env overrides are ignored
  - Test invalid temperature values
  - Test missing required fields
  - Test URL format validation
  - Test max_myses bounds
  - Test rate_limit/rate_burst bounds
  - Completed: 2026-02-05 21:30 UTC

- [x] **Add mcp proxy tests**
  - Test local tool routing
  - Test upstream fallback
  - Test auth interception
  - Completed: 2026-02-05 19:40 UTC

### 5.2 Mock Provider Delays

- [x] **Add configurable delay to MockProvider**
  - `MockProvider.SetDelay(time.Duration)`
  - Added test coverage for delay in ChatWithTools
  - Completed: 2026-02-05 19:40 UTC
...
- [x] **Analyze memory growth**
  - Run with 16 myses for 1 hour
  - Measure database size and query performance
  - Document findings
  - Completed: 2026-02-05 21:30 UTC (documented in MEMORY_GROWTH_REPORT.md)

- [x] **Implement retention policy (if needed)**
  - Not needed: DB size is not a concern for v1
  - Completed: 2026-02-05 21:30 UTC

### 6.2 MaxOpenConns=1 Bottleneck

- [x] **Benchmark concurrent writes**
  - Test with 16 myses writing simultaneously
  - Measure write latency and throughput
  - Document if acceptable for v1
  - Results: p50 0.3ms, p99 1.9ms - acceptable for v1
  - Completed: 2026-02-05 21:30 UTC

- [x] **Consider write queue (if needed)**
  - Not needed: latency acceptable with current approach
  - Completed: 2026-02-05 21:30 UTC
...
- [x] **Validate swarm config**
  - MaxMyses: 1 - 100
  - Completed: 2026-02-05 21:30 UTC

- [x] **Return validation errors from Load()**
...
- [x] **Replace Event.Data interface{}**
  - Option A: Separate event types per data struct (chosen)
  - Replaced with typed fields: Message, Error, State, Config, RateLimit
  - Completed: 2026-02-05 21:30 UTC

### 8.2 Consistent Receiver Names

- [x] **Audit and standardize**
  - Mysis: `m` (not `a`)
  - Store: `s`
  - Proxy: `p`
  - Model: `m`
...
- [x] **Add install command**
  ...
  - Completed: 2026-02-05 19:10 UTC
...
- [x] **Check Ollama logs for loop stoppage**
  ...
  - Findings: 65 HTTP 500s, 19 HTTP 400s, 3 prompt truncations, 23 client disconnects
...
| 3. Concurrency | 5 | 5 | 0 |
| 4. Error Handling | 4 | 4 | 0 |
| 5. Testing | 5 | 5 | 0 |
| 6. Database | 4 | 4 | 0 |
| 7. Configuration | 5 | 5 | 0 |
| 8. Code Quality | 3 | 3 | 0 |
